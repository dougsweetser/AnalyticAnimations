...
class Q8(np.ndarray):
    ...
    def __new__(subtype, values=None, qtype="Q", representation="", 
        ...
        obj = super(Q8, subtype).__new__(subtype, shape, dtype, buffer, offset, strides, order)
    ...
    def q_0(qtype="0", representation=""):
        ...
        q0 = Q8(values=np.array([0, 0, 0, 0]), qtype=qtype, representation=representation)
    ...
    def q_1(n=1, qtype="1", representation=""):
        ...
        q1 = Q8(values=np.array([n, 0, 0, 0]), qtype=qtype, representation=representation)
    ...
    def q_i(n=1, qtype="i", representation=""):
        ...
        qi = Q8(values=np.array([0, n, 0, 0]), qtype=qtype, representation=representation)
    ...
    def q_j(n=1, qtype="j", representation=""):
        ...
        qj = Q8(values=np.array([0, 0, n, 0]), qtype=qtype, representation=representation)
    ...
    def q_k(n=1, qtype="k", representation=""):
        ...
        qk = Q8(values=np.array([0, 0, 0, n]), qtype=qtype, representation=representation)
    ...
    def scalar(self, qtype="scalar"):
        ...
        s = Q8([self[0], self[1], 0, 0, 0, 0, 0, 0], qtype=end_qtype, representation=self.representation)
        return s
    ...
    def vector(self, qtype="v"):
        ...
        v = Q8([0, 0, self[2], self[3], self[4], self[5], self[6], self[7]], qtype=end_qtype, representation=self.representation)
        return v
    ...
    def exp(self, qtype="exp"):
        ...
        if red_t < 0:
            ...
            if (abs_v[0] == 0):
                return Q8([et, 0, 0, 0], qtype=end_qtype, representation=self.representation)
        ...
        expq = Q8([expq_dt, expq_dx, expq_dy, expq_dz], qtype=end_qtype, representation=self.representation)
    ...
    def ln(self, qtype="ln"):
        ...
        if red_t < 0:
            ...
            if (abs_v[0] == 0):
                return(Q8([math.log(red_t), 0, 0, 0], qtype=end_qtype, representation=self.representation))
        ...
        lnq = Q8([lnq_dt, lnq_dx, lnq_dy, lnq_dz], qtype=end_qtype, representation=self.representation)
    ...
    def sin(self, qtype="sin"):
        ...
        if abs_v[0] == 0:    
            return Q8([math.sin(red_t), 0, 0, 0], qtype=end_qtype, representation=self.representation)
        ...
        q_out = Q8([q_out_dt, q_out_dx, q_out_dy, q_out_dz], qtype=end_qtype, representation=self.representation)
    ...
    def tan(self, qtype="sin"):
        ...
        if abs_v[0] == 0:    
            return Q8([math.tan(red_t), 0, 0, 0], qtype=end_qtype, representation=self.representation)
    ...
    def sinh(self, qtype="sinh"):
        ...
        if abs_v[0] == 0:    
            return Q8([math.sinh(red_t), 0, 0, 0], qtype=end_qtype, representation=self.representation)
    ...
    def cosh(self, qtype="sin"):
        ...
        if abs_v[0] == 0:    
            return Q8([math.cosh(red_t), 0, 0, 0], qtype=end_qtype, representation=self.representation)
    ...
    def tanh(self, qtype="sin"):
        ...
        if abs_v[0] == 0:
            return Q8([math.tanh(red_t), 0, 0, 0], qtype=end_qtype, representation=self.representation)
...
class TestQ8(unittest.TestCase):
    ...
    def test_exp(self):
        ...
        print("exp(R): ", R_z)
        print("exp(C): ", C_z)
        self.assertTrue(Q_z.equals(Q8([1.6939227236832994, 0.7895596245415588, 1.1843394368123383, 1.5791192490831176])))
        self.assertTrue(P_z.equals(Q8([0.2836621854632263, -0.7671394197305108, 0.5753545647978831, 0])))
        self.assertTrue(R_z.equals(Q8([20.0855369231876679, 0, 0, 0])))
        self.assertTrue(C_z.equals(Q8([-4.8298093832693851, -5.5920560936409816, 0, 0])))
    ...
    def test_ln(self):
        ...
        self.assertTrue(Q_z.equals(Q8([1.7005986908310777, -0.5151902926640850, -0.7727854389961275, -1.0303805853281700])))
        self.assertTrue(P_z.equals(Q8([1.6094379124341003, 1.2566370614359172, -0.9424777960769379, 0])))
        self.assertTrue(R_z.equals(Q8([1.0986122886681098, 0, 0, 0])))
        self.assertTrue(C_z.equals(Q8([1.4978661367769954, 1.1071487177940904, 0, 0])))    
    
    @unittest.skip("Will have to investigate this one more.")
    def test_q_2_p(self):
        Q2P = self.Q.q_2_q(self.P)
        print("Q^P: ", Q2P)
        self.assertTrue(Q2P.equals(Q8([-0.0197219653530713, -0.2613955437374326, 0.6496281248064009, -0.3265786562423951])))
    ...
    def identity(dim, additive=False, non_zeroes=None, qs_type="ket"):
        ...
        if additive:
            ...
            if len(non_zeroes) != dim:
                ...
                return Q8States([Q8().q_0()], qs_type=qs_type, rows=rows, columns=columns)
