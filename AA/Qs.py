...
class Qs(object):
    ...
    quaternion series, there are $2^{dim}$ possible identities.
    """
    ...
    def __str__(self: Qs, quiet: bool = True) -> str:
        """
    ...
    def xyz(self: Qs) -> List:
        """
        Returns a 3-vector_q() for each state.
...
class Qss(object):
    """
    A class made up of many quaternion series. This structure first appeared necessary while taking a three day class
    on qubits. I figured out how to prove the Clauser, Horne, Shimony, and Holt (CHSH) inequality, a variation on 
    Bell's inequality. It is on github, https://github.com/dougsweetser/H2PQC.  

    The current reason to create this class is to compare quaternion functions, so there will be two quaternion series
    to compare, input and output.

    Quaterions are a division algebra, so form a group under addition and without zero, a group under multiplication.
    They contain as subgroups both the real and complex numbers.

    Quaternion series are a semi-group with inverses. A semi-group has more than one possible identity element. For
    quaternion series, there are $2^{dim}$ possible identities.

    At this point, I do not know if a collection of quaternion series has different properties than one quaternion series.
    """
    ...
    def __init__(self, qss=None):
        ...
        if qss is None:
            self.n, self.num, self.number = 0, 0, 0
            self.dfdf = pd.DataFrame(q0s())
        else:
            self.n, self.num, self.number = int(len(qss)), int(len(qss)), int(len(qss))
            self.dfdf = pd.DataFrame([qs for qs in qss])

    def bras(self: Qss) -> Qss:
        """
        Set all quaternion series bra form.
        ...
        for qs in self.qss:
            new_qs.append(qs.bra())

        return Qss(new_qs)

    def kets(self: Qss) -> Qss:
        """
        Set all the quaterion series to ket form.
    ...
    def __str__(self: Qss, quiet: bool = True) -> str:
        """
        Print out all the states.
...
def dupes(q_1: Q, dim: int = 2) -> Qs:
    f"""{dupe.__doc__}""".replace("Q", "Qs")
    return Qs([q_1 for _ in range(dim)])
...
def equalss(q_1: Qss, q_2: Qs, scalar: bool = True, vector: bool = True) -> bool:
    f"""{equal.__doc__}""".replace("Q", "Qs")
    ...
    for q_1q, q_2q in zip(q_1.qss, q_2.qss):
        if not equals(q_1q, q_2q, scalar, vector):
            result = False
