...
qss_1234 = Qss([qs_1234, qs_1234])
qss_q_1_q_i = Qss([q_1_q_i, q_1_q_i])
...
def test__1700_bra():
    ...
    assert equals(q_1_q_i, qz, scalar=True, vector=False)
    assert equal(qz.qs[1], qi(-1))

def test__1710_bras():
    qz = qss_q_1_q_i.bras()
    q_1_q_ic = Qs([q1(), qi(-1)])
    qss_test = Qss([q_1_q_ic,q_1_q_ic])
    qz2 = qss_q_1_q_i.bras().bras()
    qz3 = qss_q_1_q_i.bras().kets()
